...
{
	public $TitreDocument = "Jobs planifiés" ;
	public $Titre = "Jobs planifiés" ;
	...
	protected function DetermineTablPrinc()
	{
		...
		$this->TablPrinc->FournisseurDonnees->RequeteSelection = "(select t1.*, t3.titre titre_application, t2.id_application
...
order by t1.nom)" ;
		...
		$flt = $this->TablPrinc->InsereFltSelectHttpGet("date_debut", "date(date_action) >= <self>") ;
		$flt->Libelle = "Date debut" ;
		$comp = $flt->RemplaceComposant(new \Pv\ZoneWeb\FiltreDonnees\Composant\ZoneDate) ;
		$flt = $this->TablPrinc->InsereFltSelectHttpGet("date_fin", "date(date_action) <= <self>") ;
		...
		$this->TablPrinc->InsereDefColDateTimeFr("date_action", "Date") ;
		$this->TablPrinc->InsereDefCol("id_queue", "Num. queue") ;
