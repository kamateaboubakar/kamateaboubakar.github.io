...
{
	public $TitreDocument = "Demarrer le job" ;
	public $Titre = "Demarrer le job" ;
	...
	public function DetermineEnvironnement()
	{
		...
		$bd = $this->CreeBdPrinc() ;
		$this->ParamIdJob = intval(\Pv\Misc::_GET_def("id")) ;
		$this->LgnPrinc = $bd->FetchSqlRow('select t1.*, t2.id_job, t3.titre titre_application
from rpapp_job t1
left join rpapp_exec_queue_0 t2 on t1.id=t2.id_job
left join rpapp_application t3 on t1.id_application=t3.id
where t1.id=:0 and t2.id_planif is null', array($this->ParamIdJob)) ;
		...
		{
			$this->TitreDocument = "Demarrage ".$this->LgnPrinc["titre_application"]." / ".$this->LgnPrinc["nom"] ;
			$this->Titre = "Demarrage ".$this->LgnPrinc["titre_application"]." / ".$this->LgnPrinc["nom"] ;
		}
...
where t1.id_job='.$this->ParamIdJob.' and t1.actif=1 and t2.actif=1 and t3.actif=1') ;
		}
		else
		{
			$ok = $bd->RunSql("insert into rpapp_exec_queue_0
...
inner join rpapp_job t2 on t1.id_job=t2.id
left join rpapp_application t4 on t2.id_application=t4.id
where t1.id_job=".$this->ParamIdJob." and t1.actif=1 and t2.actif=1") ;
		}
		if(! $ok)
		{
